import pygame
from Old.Simple_competition.Classes.constants import WORLD_HEIGHT, WORLD_WIDTH, BACKGROUND_COLOR, SCALE_FACTOR, FOOD_COLOR, WATER_COLOR, TEXT_COLOR
...
class Screen:
    ...
    def update_text(self, best_robot, timestep, population, food, generation):
        ...
        text = self.font.render("Energy for best robot: " + str(best_robot.energy.__round__(1)), True, TEXT_COLOR)
        text_rect = text.get_rect()
        text_rect.bottomleft = 0, 40
        self.screen.blit(text, text_rect)

        text = self.font.render("Fitness for best robot: " + str(best_robot.genome.fitness.__round__(1)), True, (TEXT_COLOR))
        text_rect = text.get_rect()
        text_rect.bottomleft = 0, 60
        self.screen.blit(text, text_rect)

        text = self.font.render("Robots: " + str(len(list(filter(lambda agent: not agent.out_of_bounds, population)))), True, TEXT_COLOR)
        text_rect = text.get_rect()
