...
import random
import queue
from Old.Simple_competition.Classes.constants import WORLD_HEIGHT, WORLD_WIDTH, SCALE_FACTOR, SENSING_DISTANCE, START_POSITION, SensingMode
    ...
    def sense_box(self, simulation):
        ...
        for coord in coords:
            if coord in simulation.food:
                output.append(1)
            elif coord[0] < 0 or coord[0] > WORLD_WIDTH or coord[1] < 0 or coord[1] > WORLD_HEIGHT or coord in other_agent_coords:
                output.append(0)
            else:
                output.append(0.5)
